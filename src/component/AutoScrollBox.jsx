 ...
 *   ⚠️ El padre debe tener altura resuelta; en flex/grid usa min-height:0 en el contenedor directo.
...
export default function AutoScrollBox({
  ...
  useEffect(() => {
    ...
    const update = () => {
      const h = useVisualViewport && window.visualViewport
    ...
    const step = (t) => {
      ...
      if (!paused && speed > 0) {
        ...
        if (delta >= QUANTUM_PX || virtual === maxScroll) {
          const blocks = virtual === maxScroll
        ...
        const frac = virtual - yCommittedRef.current; // [0, QUANTUM_PX)
        inner.style.transform = frac ? `translateY(-${frac.toFixed(4)}px)` : "translateY(0px)";
        ...
        inner.style.transform = "translateY(0px)";
        yVirtualRef.current = yCommittedRef.current = boxRef.current?.scrollTop || 0;
      }
  ...
  const Wrapper = (
    <div
      ...
      style={{
        ...
        // fitParent: el wrapper también mide 100% del padre
        ...(fitParent && !fullscreen ? {
          position: "relative",
          width: "100%",
          height: "100%", // ← clave para heredar del padre
        } : {}),
        ...style,
      }}
      ...
      <div
        ...
        style={{
          height: fullscreen
            ? "100%"
            : fitParent
            ? "100%" // ← cuando fitParent=true, el scroller hereda 100% del wrapper (que hereda del padre)
